...
func (c *processorClient) Initialize(ctx context.Context, opts ...grpc.CallOption) (Processor_InitializeClient, error) {
	stream, err := c.cc.NewStream(ctx, &Processor_ServiceDesc.Streams[0], Processor_Initialize_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &processorInitializeClient{stream}
	return x, nil
}
...
func _Processor_Initialize_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ProcessorServer).Initialize(&processorInitializeServer{stream})
}
